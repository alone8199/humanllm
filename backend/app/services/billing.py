...
This module owns balance holds, usage calculation, settlement, and billing
transaction creation. The legacy ``app.billing`` module re-exports these
symbols so existing imports remain compatible during the refactor.
"""
...
def _precharge_char_limit(prompt_chars: int) -> int:
    return min(
        prompt_chars * PRECHARGE_CHARS_FACTOR + PRECHARGE_CHARS_FLOOR,
        MAX_CHARS_CAP,
    )


def precharge_cents(model: ModelConfig, prompt_chars: int) -> int:
    return _price(model, _precharge_char_limit(prompt_chars), model.timeout_seconds)
...
def compute_usage(
    ...
    capped_chars = min(completion_chars, _precharge_char_limit(prompt_chars))
...
async def settle(
    ...
    """Charge actual cost, refund the unused hold, and credit worker/platform."""
    duration = 0.0
